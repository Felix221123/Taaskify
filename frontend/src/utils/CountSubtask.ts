...
export const countIncompleteSubtasks = (
  subtasks: Subtask[],
  isCompleted: string
) => {
  ...
  const completeSubtasks = subtasks.filter(
    (subtask) => subtask.isCompleted
  ).length;
  // return `${incompleteSubtasks} of ${totalSubtasks} subtasks`;
  const text =
    isCompleted === 'completed'
      ? `${completeSubtasks} of ${totalSubtasks} subtasks`
      : `${incompleteSubtasks} of ${totalSubtasks} subtasks`;
  return text;
};
