...
export const ProfileContainer: React.FC<ProfileProps> = ({ firstName, lastName, onClickProps }) => {
  ...
  return (
    <>
      <div className="profileContainer" data-testid="profileContainer">
        <div className="avatarCircle font-normal" data-testid="avatarCircle" style={toggleBgTheme}>
          <span style={handleColorTheme}>{GetUsersFirstLetterOfNames(firstName, lastName)}</span>
        </div>
        <div onClick={onClickProps} className="cursor-pointer" data-testid="settingsIcon">
          <SettingsIcon />
        </div>
      </div>
    </>
